...
def _load_raw(paths: Paths, timep: TimeParams, tk: str) -> pd.DataFrame:
    """Загружаем базовый ряд тикера tk + экзогенные (Brent, USD/RUB) без ключевой ставки."""
    if tk not in TICKERS:
        raise KeyError(f"Неизвестный тикер {tk} в TICKERS.")
    base = fetch_moex(
        paths.cache_dir,
        tk,
        *TICKERS[tk],
...
def _timeline_from_full_preds(tk: str, *dirs: str) -> pd.DatetimeIndex:
    ...
    for out_dir in dirs:
        ...
        for fname in os.listdir(full_dir):
            if not fname.endswith(".csv") or "_f" not in fname:
                continue
            if not fname.startswith(f"{tk}_"):
                continue
...
def _load_predictions(tk: str, out_dir: str, suffix: str) -> Dict[str, pd.Series]:
...
def build_master(out_dir: str, dest_prefix: str, tickers: Iterable[str]) -> None:
    paths = Paths(out_dir=out_dir)
    timep = TimeParams()

    for tk in tickers:
        raw = _load_raw(paths, timep, tk)
        ...
        tl = _timeline_from_full_preds(tk, out_dir, "outputs_logret", out_dir.replace("outputs", "outputs_logret"))
        if tl.empty:
            tl = feat_d.index
        ...
        # Базовые уровни и факторы (feat_d содержит индикаторы и лаги)
        ...
        # Кластеры режимов
        cl_path = os.path.join(out_dir, "clustering", f"{tk}_clusters.csv")
        if os.path.exists(cl_path):
            ...
            name_map = {}
            stats_path = os.path.join(out_dir, "clustering", f"{tk}_cluster_stats.csv")
        ...
        pred_d = _load_predictions(tk, out_dir, suffix="_dclose")
        for col, series in pred_d.items():
            master[col] = series.reindex(timeline)
        # logret прогнозы из outputs_logret
        pred_l = _load_predictions(tk, "outputs_logret", suffix="_logret")
        for col, series in pred_l.items():
            master[col] = series.reindex(timeline)

        master.reset_index(inplace=True)
        master.rename(columns={"index": "Datetime"}, inplace=True)
        dest = f"{dest_prefix}_{tk}.csv"
...
def main():
    ...
    ap.add_argument("--dest", default="outputs/master_table", help="Префикс для сохранения финальных таблиц (добавляется _{ticker}.csv)")
    ap.add_argument("--tickers", nargs="*", default=list(TICKERS.keys()), help="Список тикеров (по умолчанию все из TICKERS)")
    args = ap.parse_args()
    build_master(args.out_dir, args.dest, args.tickers)
