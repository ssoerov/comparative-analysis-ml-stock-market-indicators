...
def main():
    m = pd.read_csv('outputs/consolidated/metrics_all.csv')
    e = pd.read_csv('outputs/consolidated/economics_all.csv')
    agg_dict = {
    ...
    print('=== Aggregate metrics per ticker/model (ALL) ===')
    ...
    try:
        dm = pd.read_csv('outputs/consolidated/dm_test_pairs_all.csv')
        print('\n=== Pairwise DM tests (first 30 rows) ===')
        print(dm.head(30).to_string(index=False))
